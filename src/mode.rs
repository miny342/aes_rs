...
pub trait BlockCipher<const TEXT_SIZE: usize> {
    ...
    fn encrypt_cfb(&self, in_bytes: &[u8], iv: [u8; TEXT_SIZE], out_bytes: &mut [u8]) {
        self._prologue(in_bytes, out_bytes);
        let mut e = self._encrypt(iv);
        out_bytes[0..TEXT_SIZE].copy_from_slice(&array::from_fn::<u8, TEXT_SIZE, _>(|i| in_bytes[i] ^ e[i]));
        for (i, v) in in_bytes[TEXT_SIZE..].windows(TEXT_SIZE).step_by(TEXT_SIZE).enumerate() {
            e = self._encrypt(out_bytes[i * TEXT_SIZE..(i + 1) * TEXT_SIZE].try_into().unwrap());
            out_bytes[(i + 1) * TEXT_SIZE..(i + 2) * TEXT_SIZE].copy_from_slice(&array::from_fn::<u8, TEXT_SIZE, _>(|j| v[j] ^ e[j]))
        }
    }
    fn decrypt_cfb(&self, in_bytes: &[u8], iv: [u8; TEXT_SIZE], out_bytes: &mut [u8]) {
        ...
        let mut e = self._encrypt(iv);
        out_bytes[0..TEXT_SIZE].copy_from_slice(&array::from_fn::<u8, TEXT_SIZE, _>(|i| in_bytes[i] ^ e[i]));
        for (i, v) in in_bytes.windows(2 * TEXT_SIZE).step_by(TEXT_SIZE).enumerate() {
            e = self._encrypt(v[0..TEXT_SIZE].try_into().unwrap());
